...
const ActionColumn = ({ row }) => {
    const INIT_TEMAS = {
        IdCategorias: row.cells[1].value ? row.cells[1].value : row.cells[1].value,
    ...
    const actualizar = (event) => {
        ...
        if (validated) {
            ...
            const url = `${environment.baseURL}accion=instcategorias&opcion=actualizar&${response}`;
...
const FormActividades = (props) => {
    ...
    const guardar = (event) => {
        ...
        if (validated) {
            ...
            const url = `${environment.baseURL}accion=instcategorias&opcion=guardar&${response}`;
            const respuesta = api.getDatos(`${url}`);
            respuesta.then(function (resp) {
                if (resp) {
                  Swal.fire('' + resp[0].menssage + '');
...
const ListCategorias = (props: categorias): React$Element<any> => {
